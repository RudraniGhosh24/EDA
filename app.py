...
def load_and_preprocess_data():
    # 1. Load Data
    df_crimes = pd.read_csv("all_done_2.csv")
    ...
    df_crimes['state'] = df_crimes['state'].replace({'MADHYA PRADESH  ': 'MADHYA PRADESH'})
    ...
    # Assuming 'Female' column is Female Population from census snapshot
    # To address Year vs Census misalignment, we'd ideally interpolate population for each year.
    # As a simple proxy here, we use the static census population to compute rates.
    df['Female Population'] = df['Female'].fillna(df['Male Population']) # Fallback if Female is missing
    df['Total Population'] = df['persons'].fillna(df['Male Population'] * 2) 

    # Calculate Crime Rate (Crimes per 100,000 women)
    # Using 'Total Cognizable Offences' as the sum or calculating it
    ...
    # Ensure all crime cols are numeric
    for c in crime_cols:
    ...
    # Normalized Rates
    df['Crime Rate (per 100k women)'] = (df['Total Crimes Against Women'] / df['Female Population']) * 100000
    
    # Other engineered features
    df['Gender Ratio (F per 1000 M)'] = (df['Female'] / df['Male']) * 1000
    df['Literacy Gap'] = df['Male Literacy Rate'] - df['Female lit_Rate']
    df['Urbanization Rate (%)'] = (df['urban_population'] / df['Total Population']) * 100
...
# MAIN DASHBOARD
# -----------------
tab1, tab2, tab3, tab4 = st.tabs(["Overview & Rates", "Geospatial Map", "Crime Trends", "Feature Correlation"])
...
with tab1:
    ...
    # Bar chart of top districts by Crime Rate vs Absolute
...
with tab2:
    ...
    try:
        ...
        # Mapping state names to match GeoJSON
        # GeoJSON has Title Case state names mostly
        state_agg['state_title'] = state_agg['state'].str.title()
        # Some manual corrections to match GeoJSON properties
...
with tab4:
    ...
    features = ['Crime Rate (per 100k women)', 'Total Crimes Against Women', 'Urbanization Rate (%)', 'Literacy Gap', 'Gender Ratio (F per 1000 M)', 'Male Literacy Rate', 'Female lit_Rate']
    ...
    if available_features:
        ...
        c1, c2 = st.columns(2)
        with c1:
            x_axis = st.selectbox("X-Axis", ['Urbanization Rate (%)', 'Literacy Gap', 'Gender Ratio (F per 1000 M)'])
        with c2:
            y_axis = st.selectbox("Y-Axis", ['Crime Rate (per 100k women)', 'Total Crimes Against Women'])
            
        scatter_df = filtered_df.groupby(['state', 'District'])[available_features].mean().reset_index()
